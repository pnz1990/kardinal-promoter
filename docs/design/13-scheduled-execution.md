...
- 🔲 **Onboarding first-run success is not verified — scaffolding generation ≠ working loop** — the `/otherness.onboard` command is considered "done" when it generates `otherness-config.yaml`, the workflow YAML, and design doc stubs. There is no step that verifies the generated artifacts produce a working first scheduled run. The gap: `otherness-config.yaml` could have incorrect values for `report_issue`, `pr_label`, or `agents_path`; the workflow could have a YAML structure error; the required secrets may not be set. A first-run failure after onboarding is invisible to the onboarding agent because the agent exits after file generation, long before the first cron tick fires. Add a post-generation smoke test to the onboard agent: (1) run `bash -n` on the generated workflow; (2) run `gh api repos/$REPO` to verify the repo exists and is accessible with the provided token; (3) run `gh secret list --repo $REPO` and verify each required secret name is present; (4) run `python3 -c "import yaml; yaml.safe_load(open('otherness-config.yaml'))"` to verify YAML validity. Report the results as `[ONBOARD SMOKE TEST: 4/4 checks passed]` or list the failures. This converts "I generated the files" into "I verified the files will work." ⚠️ Inferred from pressure lens: onboarding produces docs that need manual editing — the smoke test is the minimum viable verification gate.
