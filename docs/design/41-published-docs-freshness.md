...
- ✅ **QA docs gate** `scripts/qa-docs-gate.sh` reads a PR diff, detects Future→Present transitions in `docs/design/*.md`, classifies user-visible features (CLI/CRD/UI keywords), verifies `docs/*.md` files were updated or feature is Layer 1 auto-documented. Exits 1 with WRONG finding if missing. Fail-open. (PR #1134, 2026-04-23)
