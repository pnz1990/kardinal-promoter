...
- docs(contributing): document qa-docs-gate.sh usage and exit codes (#1148)
- fix(controller): readyz probe gates on informer cache sync — fixes PDCA S1 flap (#1147)
- feat(qa): add qa-docs-gate.sh — QA §3b docs gate for user-visible features (#1144)
- test(graph): add single-region edge-case test for BuildNodes (#1111) (#1141)
- feat(pm): add changelog-completeness.sh — PM §5n-changelog check (#1135) (#1140)
- feat(monitoring): ship Grafana dashboard JSON and Helm ConfigMap template (#1128) (#1139)
- 3 PRs merged; PDCA root causes fixed (#1138)
- docs(design): update doc-15 — mark runbook URLs and PDCA items as resolved (#1137)
- fix(ci): regenerate CRD schemas and fix gofmt alignment in promotionstep reconciler (#1126)
...
- feat(crd): add ResourceRef to HealthConfig — fix PDCA S9 unknown field error (#1117)
- fix(ci): pin agent_version to v0.3.0 to stop failing upgrade path (#1115)
- No changes; 75-item backlog; PDCA failing x5. (#1114)
...
- fix(workflow): commit agent_version upgrade to session branch (#1109)
- Step A done; 0 changes; PDCA failing x5. (#1108)
- fix(workflow): fix agent_version/upgrade_policy regex (#1107)
- feat(loop): add zero-pr-detect.sh — detect sessions with no merged PRs (#1106)
- feat(loop): add adversary-check.sh — monoculture break for architecture reviews (#1105)
- Vision scan done; 1 promoted, 77 backlog (#1104)
- feat(docs): define explicit SM health thresholds GREEN/AMBER/RED/STALL (#1103)
- feat(ci): add onboard smoke-test script — 4-check post-onboard validation (#1102)
