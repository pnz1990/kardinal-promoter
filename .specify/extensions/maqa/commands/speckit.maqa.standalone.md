---
description: "MAQA Standalone Agent. Single session plays all roles sequentially: coordinator → engineer → QA (adversarial) → coordinator → SM → PM → repeat. One item at a time, fully autonomous. Use when running a single session instead of the full team."
---
...
No generic MAQA standalone implementation. Set `agents_path` in `maqa-config.yml` to provide one.
